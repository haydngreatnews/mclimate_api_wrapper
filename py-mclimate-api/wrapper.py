...
class MClimate(CoreDevices):
    ...
    def send(self, device, device_type='melissa', state_data=None):
        if not self._send_cache:
            ...
            if device_type == 'smart_plug':
                data = self.DEFAULT_DATA_SMARTPLUG.copy()
        else:
            data = self._send_cache.copy()
        if state_data:
            data.update(state_data)
        data.update({self.SERIAL_NUMBER: device})
        url = MCLIMATE_URL % 'provider/send'
        LOGGER.info(url)
        headers = self._get_headers()
    ...
    def status(self, test=False, cached=False):
        ...
        url = MCLIMATE_URL % 'provider/fetch'
        LOGGER.info(url)
        headers = self._get_headers()
        headers.update({'Content-Type': 'application/json'})
        ret = {}
        ...
        for device in self.devices:
            if self.devices[device]['type'] in ('melissa', 'bobbie', 'smart_plug'):
                ...
                if req.status_code == requests.codes.ok:
                    ...
                    if self.devices[device]['type'] == 'bobbie' or self.devices[device]['type'] == 'smart_plug':
    ...
    def cur_settings(self, serial_number):
        url = MCLIMATE_URL % 'controllers/%s' % serial_number
        LOGGER.info(url)
        headers = self._get_headers()
        req = SESSION.get(
                url, headers=headers)
